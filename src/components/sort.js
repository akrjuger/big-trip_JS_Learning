...
export default class SortingComponent extends AbstractComponent {
  ...
  hideDayTitle() {
    // this.getElement().querySelector(`.trip-sort__item--day`).classList.add(HIDDEN_CLASS);
    this.getElement().querySelector(`.trip-sort__item--day`).innerText = ``;
  }

  showDayTitle() {
    const title = this.getElement().querySelector(`.trip-sort__item--day`);
    if (title.innerText === ``) {
      title.innerText = `Day`;
    }
  }
